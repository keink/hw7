...
class Game:
	...
	def Evaluate(self):
		...
		for y in xrange(1,8):
			for x in xrange(1,8):
				if self.Pos(x,y)==1:
					score+=score1[y-1][x-1]
				elif self.Pos(x,y)==2:
					score-=score1[y-1][x-1]
		return score

	def Score(self,depth,moves):
		if depth<1:
		...
		if len(moves)!=0:
			for move in moves:
				...
				next_valid_moves=nextboard.ValidMoves()
				score=nextboard.Score(depth-1,next_valid_moves)
			...
			self._board["Next"]=3-player
			nextboard=self
			next_valid_moves=nextboard.ValidMoves()
			nextboard.Score(depth-1,next_valid_moves)
		if depth!=3:
			return best_score
		else:
			return best_move
...
""")
    ...
    def pickMove(self, g):
    	...
    	if len(valid_moves) == 0:
	    	...
	    	best_move = g.Score(3,valid_moves)
